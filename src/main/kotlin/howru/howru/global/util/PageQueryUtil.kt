...
): BooleanExpression? = lastId?.takeIf { it > 0 }?.let { idPathExtractor(qEntity).lt(it) }

fun <T, Q : EntityPathBase<T>> ltLastTimestamp(
    lastTimestamp: Int?,
    qEntity: Q,
    timestampPathExtractor: (Q) -> NumberPath<Int>
): BooleanExpression? = lastTimestamp?.takeIf { it > 0 }?.let { timestampPathExtractor(qEntity).lt(it) }

fun <T> findLastIdOrDefault(
    foundData: List<T>,
    idExtractor: (T) -> Long
): Long = foundData.lastOrNull()?.let { idExtractor(it) } ?: 0
